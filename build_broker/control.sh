...
function start() {
    ...
    PULSAR_BROKER_CONF="${PULSAR_HOME}/conf/broker.conf"
    ...
    PULSAR_BROKER_CONF=${CLUSTER_NAME}"."${SERVICE_NAME}

    if [[ ${CLUSTER_NAME} == hna-pre* ]]; then
      PULSAR_BROKER_CONF="${PULSAR_HOME}/conf/broker_conf/broker.preview.conf"
    fi
   ...
   #for docker machine, change default memory limit
    ...
    echo "  PULSAR_BROKER_CONF:" $PULSAR_BROKER_CONF
    ...
    echo "  advertisedAddress:" $advertisedAddress
